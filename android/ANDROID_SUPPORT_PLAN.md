...
> Status: **plan only**. No Android SDK/NDK is installed on the dev host (verified: `android`, `sdkmanager`, `ndk-build`, `gradle`, `aapt2`, `apksigner`, `zipalign`, `d8` all absent; no `ANDROID_HOME`/`NDK_HOME`), so the APK cannot be built or verified locally in this session. The first build must happen either after installing the NDK + SDK build-tools locally, or inside a new CI job (preferred, mirrors existing release pipeline).
